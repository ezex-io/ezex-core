...
pub trait EventBus: Sized + Send + Sync {
    ...
    //     };
    //     let res = Stream::new(msg.event(), None, fields.encode()?);
    ...
    //     key: &str,
    //     msg: &str,
    // ) -> anyhow::Result<Vec<Box<dyn EventMessage>>>;
...
//     #[derive(Serialize, Deserialize, Debug, Clone, Event, PartialEq, Eq)]
...
//         let req_stream = Stream::new(req.event(), stream_id.clone(), fields.encode().unwrap());
